...
{
    /// <summary>
    /// Deterministic cargo-damage model layered onto the existing delivery evaluation.
    /// It does not alter truck physics; damage is derived from delivery quality and cargo traits.
    /// </summary>
    public static class CargoDamageSystem
    {
        public static float CalculateDamagePercent(CargoDefinition cargo, float deliveryScore, float dockingScore)
        {
        ...
        public static float GetPayoutPenalty(float reward, float damagePercent)
        {
            float severity = Mathf.Clamp01(damagePercent / 35f);
            return Mathf.Max(0f, reward) * severity * 0.30f;
        }
