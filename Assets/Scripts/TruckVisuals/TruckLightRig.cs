...
namespace UltimateTruckEmpire.Visuals
{
    ...
    /// <summary>
    /// Presentation-only lighting for the truck and trailer.
    ///
    /// Design rules:
    ///  * Lamps are emissive material swaps, not lights. Swapping two cached
    ///    shared materials costs nothing and never allocates per-instance
    ///    materials, so a yard full of trucks stays cheap.
    ///  * At most TWO realtime lights per vehicle (the headlamp spots), both
    ///    with shadows off, and they only exist while the headlights are on.
    ///  * The optional cab light is a single low-range point light, off by default.
    ///  * No per-frame Update: state changes are pushed, and the indicator blink
    ///    runs on a coroutine.
    ///
    /// If the project already has a TruckLights script, keep it - let it drive
    /// this rig by calling SetHeadlights/SetBrakes/SetIndicator instead of
    /// creating a second lighting system.
    /// </summary>
    ...
    public class TruckLightRig : MonoBehaviour
    {
        ...
        // Cached lens materials (shared, never instanced).
        ...
        // ------------------------------------------------------------------
        // Public API - safe to call from TruckController / TruckLights
        // ------------------------------------------------------------------
        ...
        public void SetIndicator(IndicatorState state)
        {
            ...
            if (state != IndicatorState.None && isActiveAndEnabled)
                _blink = StartCoroutine(BlinkRoutine());

            Refresh();
        }

        /// <summary>Convenience for controllers that only have running-lights state.</summary>
        public void SetRunningLights(bool on)
        {
            SetHeadlights(on ? HeadlightMode.Low : HeadlightMode.Off);
        }
        ...
        {
            // Hard cap: two realtime lights, no shadows.
            int made = 0;
            for (int i = 0; i < headlightAnchors.Count && made < 2; i++)
            {
                Transform anchor = headlightAnchors[i];
        ...
        private void Refresh()
        {
            ...
            // Tail first, then brake overrides the shared rear lenses.
        ...
        /// <summary>Merges another light set in (used to attach a trailer's lamps).</summary>
