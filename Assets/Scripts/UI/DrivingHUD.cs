...
namespace UltimateTruckEmpire.UI
{
    public sealed class DrivingHUD : MonoBehaviour
    {
        ...
        private Button MakeButton(Transform parent, string label, UnityEngine.Events.UnityAction action)
        {
            ...
            Text text = new GameObject("Text").AddComponent<Text>(); text.transform.SetParent(go.transform, false); text.font = font; text.fontSize = 15; text.alignment = TextAnchor.MiddleCenter; text.color = Color.white; text.text = label; Stretch(text.rectTransform); return button;
        }
