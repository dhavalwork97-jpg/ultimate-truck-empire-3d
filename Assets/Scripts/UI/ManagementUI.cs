...
namespace UltimateTruckEmpire.UI
{
 public sealed class ManagementUI:MonoBehaviour
 {
  ...
  private static void Contracts(StringBuilder s){var m=ContractMarket.Instance;if(m==null)return;int completed=DeliveryManager.Instance?.CompletedContracts??0;s.AppendLine($"Route Progression: {RouteProgression.GetTierLabel(RouteProgression.CurrentTier(completed))}");s.AppendLine($"Completed Deliveries: {completed}");s.AppendLine();foreach(var c in m.Offers)s.AppendLine($"[{RouteProgression.GetTierLabel(c.routeTier)}] {c.cargo} | {c.pickup} → {c.destination} | {c.weightTons:0.0}t | {c.distanceKm:0} km | {c.trailerType} | {ContractModifierRules.GetLabel(c.modifier)} | ₹{c.reward:0} | {(string.IsNullOrWhiteSpace(c.customerName) ? "OPEN MARKET" : c.customerName)}");}private static void Finances(StringBuilder s){var f=FinanceManager.Instance;var g=GameManager.Instance;if(f==null||g==null)return;s.AppendLine($"Cash ₹{g.Money:0}");s.AppendLine($"Revenue ₹{f.Data.revenue:0}");s.AppendLine($"Operating Expenses ₹{f.TotalOperatingExpenses:0}");s.AppendLine($"Operating Profit ₹{f.OperatingProfit:0}");s.AppendLine($"Capital Investment ₹{f.Data.capitalExpense:0}");s.AppendLine($"Total Cash Outflow ₹{f.TotalExpenses:0}");s.AppendLine($"Debt ₹{f.Data.debt:0}");
  s.AppendLine($"Fuel market: Ahmedabad ₹{FuelPriceManager.Instance?.GetPricePerLitre("Ahmedabad") ?? EconomyConfig.FuelPricePerLitre:0.00}/L | Vadodara ₹{FuelPriceManager.Instance?.GetPricePerLitre("Vadodara") ?? EconomyConfig.FuelPricePerLitre:0.00}/L | Surat ₹{FuelPriceManager.Instance?.GetPricePerLitre("Surat") ?? EconomyConfig.FuelPricePerLitre:0.00}/L | Kandla ₹{FuelPriceManager.Instance?.GetPricePerLitre("Kandla") ?? EconomyConfig.FuelPricePerLitre:0.00}/L");}
  ...
  private static void Jobs(StringBuilder s){var jobs=AutomatedDeliveryManager.Instance?.ActiveDeliveries;if(jobs==null)return;foreach(var j in jobs)s.AppendLine($"{j.id} | {j.cargo} | {j.trailerType} | {ContractModifierRules.GetLabel(j.modifier)} | {j.origin} → {j.destination} | Remaining {j.remainingKm:0} km | ETA {j.etaHours:0.0} h | ₹{j.reward:0}");if(jobs.Count==0)s.AppendLine("No active automated deliveries.");}
 }
}
