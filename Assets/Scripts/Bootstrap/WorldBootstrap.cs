...
namespace UltimateTruckEmpire.Bootstrap
{
    public sealed class WorldBootstrap : MonoBehaviour
    {
        ...
        {
            RenderSettings.ambientIntensity = 1.1f; RenderSettings.fog = true; RenderSettings.fogDensity = .004f; CreateLight();
            CreateRoad(new Vector3(0, -.15f, 0), new Vector3(180, .3f, 14)); CreateRoad(new Vector3(0, -.15f, 70), new Vector3(180, .3f, 14)); CreateRoad(new Vector3(0, -.15f, -70), new Vector3(180, .3f, 14)); CreateRoad(new Vector3(-90, -.15f, 35), new Vector3(14, .3f, 140));
            WorldVisualBuilder.Build(); CreateDepot(new Vector3(-55, 0, 0), "Ahmedabad Logistics Depot", DeliveryTrigger.TriggerType.Pickup); CreateDepot(new Vector3(55, 0, 0), "Vadodara Factory Warehouse", DeliveryTrigger.TriggerType.Destination); CreateTruck(new Vector3(-20, 1.1f, 0)); CreateHud();
        }
        ...
        { var building = GameObject.CreatePrimitive(PrimitiveType.Cube); building.name = name; building.transform.position = position + new Vector3(0, 5, 12); building.transform.localScale = new Vector3(28, 10, 18); var zone = new GameObject(name + " Zone"); zone.transform.position = position + Vector3.up; var box = zone.AddComponent<BoxCollider>(); box.isTrigger = true; box.size = new Vector3(22, 3, 12); zone.AddComponent<DeliveryTrigger>().Configure(type); }
        private static void CreateTruck(Vector3 position)
        {
            var truck = new GameObject("Player Truck"); truck.tag = "PlayerTruck"; truck.transform.position = position; var body = truck.AddComponent<Rigidbody>(); body.mass = 8000; body.centerOfMass = new Vector3(0, -.7f, 0); var collider = truck.AddComponent<BoxCollider>(); collider.center = new Vector3(0, 1.2f, 0); collider.size = new Vector3(3, 2.4f, 7);
            ...
            truck.AddComponent<TrailerController>().Configure(TrailerType.DryVan); truck.AddComponent<TruckController>(); truck.AddComponent<TruckPhysics>(); truck.AddComponent<TruckInput>(); truck.AddComponent<TruckWheelRig>();
        ...
        {
            var canvasGo = new GameObject("HUD"); var canvas = canvasGo.AddComponent<Canvas>(); canvas.renderMode = RenderMode.ScreenSpaceOverlay; canvasGo.AddComponent<CanvasScaler>(); canvasGo.AddComponent<GraphicRaycaster>(); var textGo = new GameObject("HUD Text"); textGo.transform.SetParent(canvasGo.transform); var text = textGo.AddComponent<Text>(); text.font = Resources.GetBuiltinResource<Font>("LegacyRuntime.ttf"); text.fontSize = 22; text.alignment = TextAnchor.UpperLeft; text.text = "ULTIMATE TRUCK EMPIRE\n\nWASD / Arrows Drive   SPACE Brake   I Engine   L Lights   Q/R Indicators\nF1 Fleet   F2 Drivers   F3 Contracts   F4 Finances   F5 Company   F6 Active Jobs\nF7 Weather   D Dealership\n\nSTART: Drive to the marked Ahmedabad depot to load cargo, then deliver to Vadodara.\nDRIVE THE TRUCK. BUILD THE COMPANY. CREATE THE EMPIRE."; var rt = text.rectTransform; rt.anchorMin = new Vector2(0, 1); rt.anchorMax = new Vector2(0, 1); rt.pivot = new Vector2(0, 1); rt.anchoredPosition = new Vector2(24, -24); rt.sizeDelta = new Vector2(900, 360); DeliveryManager.Instance?.AcceptStarterContract();
        }
    }
}
